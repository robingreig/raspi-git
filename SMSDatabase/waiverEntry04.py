...
#####	If they haven't signed in add an entry to the database
    if waiverSign == 0:
        try:
            ...
            cursor.execute("INSERT into users (date,  saitID, firstName, lastName, phone, email1, email2, school, program, active, waiver, mentor) \
                   VALUES (date('now', 'localtime'),?,?,?,?,?,?,?,?,1,1,0)", (sait, name1, name2, phoneNum, saitEmail, otherEmail, saitSchool, saitCourse))
            dbc.commit()
            print("\n",name1,name2," is now entered into the MakerSpace Database!")
            time.sleep(delay2)
            waiverSign = 1
            if debug > 0:
                print ("\nDatabase Updated")
...
#####           print("Error: {}".format(error))
           dbc.rollback()
           print("\n***** Entry was cancelled due to >>>>> {}".format(error),"<<<<< and going back to the main screen *****")
           time.sleep(delay2)
           time.sleep(delay2)
        finally:
            cursor.close()
