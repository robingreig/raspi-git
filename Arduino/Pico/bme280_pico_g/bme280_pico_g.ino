...
/*****
 * bme280_pico_g from Random Nerd Tutorials who got it from Adafruit
...
#define SEALEVELPRESSURE_HPA (1024.25) // for Irricana (935m ASL)
//#define SEALEVELPRESSURE_HPA (1013.25) // for Portugal (RNT)
//#define SEALEVELPRESSURE_HPA (1002.5) // for Calgary (?)
...
void pubPres() {
    ...
    client.publish(topic2, tempPres,"-r" ); //publish pressure
}

void pubAlt() {
    bme280Alt = (bme.readAltitude(SEALEVELPRESSURE_HPA));
    Serial.print("Altitude: ");
    Serial.print(bme280Alt);
    Serial.println(" m");
    dtostrf(bme280Alt, 6, 2, tempAlt);
    Serial.print("tempAlt = ");
    Serial.println(tempAlt);
    client.publish(topic3, tempAlt,"-r" ); //publish altitude
}
...
void pubHum() {
    ...
    Serial.println(" %");
    dtostrf(bme280Hum, 6, 2, tempHum);
    Serial.print("tempHum = ");
    Serial.println(tempHum);
    client.publish(topic4, tempHum,"-r" ); //publish humidity
}
