...
/*
  SolidStateRelay_mqtt_8ch_ESP8266-24
  Robin Greig, 2025.12.12
  Use 8 channel Solid Stare Relay board with ESP8266 MCU on terminal block
  mqtt keepalive is only 15 seconds so I've added 2 functions
  to check for wifi & mqtt connection at the start of the loop
  mqtt broker: mqtt43.local
  mqtt topic: esp8266/24/GPIO
...
int pinNum[8] = {16, 05, 04, 00, 14, 12, 13, 15};
// Initialize Relay 1, GPIO16, D0, WAKE pin as an output
// Initialize Relay 2, GPIO05, D1, SCL pin as an output
// Initialize Relay 3, GPIO04, D2, SDA pin as an output
// Initialize Relay 4, GPIO00, D3, FLASH pin as an output
// Initialize Relay 5, GPIO14, D5, SCLK pin as an output
// Initialize Relay 6, GPIO12, D6, MISO pin as an output
// Initialize Relay 7, GPIO13, D7, MOSI pin as an output
// Initialize Relay 8, GPIO15, D8, CS pin as an output

const char *rssi = "esp8266/24/RSSI"; // MQTT publish topic for signal RSSI
...
//    if (client.connect("ESP8266Client", mqttUser, mqttPassword )) {
    if (client.connect(client_id.c_str())) {
      ...
      client.subscribe("esp8266/24/GPIO"); // subscribe to mqtt channel
    } else {
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  Serial.println();
  // Solid State Relays turn on with a 0 sent and off with a 1 sent
  // Switch on Relay 1 & the BuiltInLED if a 0 was received as the first character
  if ((char)payload[0] == '0') {
