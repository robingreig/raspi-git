...
echo " <table style='width: 80%; text-align: left; margin-left: auto; margin-right: auto;' border='0' cellpadding='2' cellspacing='2'><tr><th>Date</th><th>Garden#1</th><th>Garden#2</th></th><th>Garden#3</th><th>Garden#4</th><th>Garden#5</th><th>Garden#6</th></tr>";

// For loop to scroll through todays values & 9 previous by incrementing the interval of the CURDATE below
for ($i = 0; $i <=0; $i++){

 $mintemp00 = mysql_query("SELECT date, min(garden_temp1), min(garden_temp2) FROM garden_temp WHERE date= (CURDATE() - INTERVAL $i DAY)");
 $maxtemp00 = mysql_query("SELECT date, max(garden_temp1), max(garden_temp2) FROM garden_temp WHERE date= (CURDATE() - INTERVAL $i DAY)");
// While loop to pull out the min & max values of each voltage
 while(($row = mysql_fetch_array($mintemp00))and ($row1 = mysql_fetch_array($maxtemp00))){
   $slash = " / ";
   $mintemp0 = ROUND($row['min(garden_temp1)'],2);
   $mintemp1 = ROUND($row['min(garden_temp2)'],2);
   $maxtemp0 = ROUND($row1['max(garden_temp1)'],2);
   $maxtemp1 = ROUND($row1['max(garden_temp2)'],2);
...
//  echo "<tr><td>" . $row['date'] . "</td><td>" . ($mintemp0.$slash.$maxtemp0) . "</td><td>" . ($mintemp1.$slash.$maxtemp1) . "</td><td>" .($mintemp2.$slash.$maxtemp2) . "</td><td>" . ($mintemp3.$slash.$maxtemp3) . "</td><td>" . ($mintemp4.$slash.$maxtemp4) . "</td><td>" . ($mintemp5.$slash.$maxtemp5) . "</td><td>" . ($mintemp6.$slash.$maxtemp6) . "</td></tr>";
  echo "<tr><td>" . $row['date'] . "</td><td>" . ($mintemp0.$slash.$maxtemp0) . "</td><td>" . ($mintemp1.$slash.$maxtemp1) . "</td><td>" .($mintemp2.$slash.$maxtemp2) .  "</td></tr>";
 }
}
