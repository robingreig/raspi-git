...
class Zoo:
    ...
    def findCanine(self):
        canineCheck = ['Canines','Wolf']
        canineSet = list(map(lambda  animal: animal.look() , self.__animal_list)) # create list from map object of animals
#        canineList = functools.reduce(lambda a,b: b, canineSet) # Reduce list
        canineList = list(filter((lambda a,b: b, canineSet), canineCheck)) # Reduce list
        print('\n===Canine Found===\n',canineList) # Print reduced list
