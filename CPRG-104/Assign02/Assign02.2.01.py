...
class Zoo:
    ...
    def looking(self):
        ...
        for bird in self.__bird_list:
            print(bird.look())
            print("")
        zooAnimals = map(lambda animal: animal.look(), self.__animal_list) # create map object of animals
        zooBirds = map(lambda bird: bird.look(), self.__bird_list) # create map object of bird
        zooAnimals1 = list(zooAnimals) # convert from map object to list
        zooBirds1 = list(zooBirds) # convert from map object to list
        self.__zooString.append(zooAnimals1) # append zooAnimals1 to full zoo string
        self.__zooString.append(zooBirds1) # append zooBirds1 to full zoo string
        prod = functools.reduce(lambda a,b: a+b, self.__zooString) # combine both strings using reduce
        print(list(prod)) # print combined string
        
    def canineLooking(self):
        canineSet = lambda animal: animal.look(), self.__animal_list
        print('canineSet: ',list(canineSet))
        for animal in self.__animal_list:
            print(animal.look())
            print("")
        canineSet1 = filter(lambda animal: animal == 'Wolf', canineSet)
        print('canineSet1: ',list(canineSet1))
