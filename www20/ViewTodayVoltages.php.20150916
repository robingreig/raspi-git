...
$result = mysql_query("SELECT * from voltages WHERE date = CURDATE() ORDER BY time DESC");

//Table starting tag and header cells
echo " <table style='width: 80%; text-align: left; margin-left: auto; margin-right: auto;' border='0' cellpadding='2' cellspacing='2'><tr><th>Date</th><th>Time</th><th>Test Battery</th><th>Grey Truck</th></th><th>Voltage2</th><th>Voltage3</th><th>Voltage4</th><th>Voltage5</th><th>Voltage6</th><th>Voltage7</th><th>Voltage8</th><th>Voltage9</th></tr>";
while($row = mysql_fetch_array($result)){
   //Display the results in different cells
   echo "<tr><td>" . $row['date'] . "</td><td>" . $row[time] . "</td><td>" . $row['voltage0'] . "</td><td>" . $row['voltage1'] . "</td><td>" . $row['voltage2'] . "</td><td>" . $row['voltage3'] . "</td><td>" . $row['voltage4'] . "</td><td>" . $row['voltage5'] . "</td><td>" . $row['voltage6'] . "</td><td>" . $row['voltage7'] . "</td></tr>";
}
//Table closing tag
echo "</table>";
